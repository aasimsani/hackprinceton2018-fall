...
def master(gcs_uri):
    ...
    def parse_results(annotations):
        ...
        for index, sentence in enumerate(annotations.sentences):
            ...
            return (sentence_sentiment,magnitude)
    ...
    for data in speakers:
        ...
        if isinstance(raw_data,type(None)):
            ...
            sentiment = float(raw_data[0])
            magnitude = float(raw_data[1])


            if (sentiment - 0.1) > 0.01 and abs(magnitude) > 0.15:
                sentiment_enc = 2
            elif sentiment <= 0.01 and sentiment > 0 and abs(magnitude) < 0.3:
                sentiment_enc = 0
            elif  (sentiment - 0.1) < 0.10 and abs(magnitude) > 0.15:
               sentiment_enc = -2
            else:
                sentiment_enc = 5
        ...
        if data['speaker'] == 1:
            ...
            speaker_sentiment[2].append(data_other)
        else:
            speaker_sentiment[1].append(data_other)
